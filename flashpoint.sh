...
entry_query=$(sqlite3 --separator $'\n' "file:${database_file}?mode=ro" ".param set :id $entry_id" "select path,platformName,game_data.applicationPath,game_data.launchCommand from game_data join game on game_data.gameId = game.id where gameId = :id" 2> /dev/null)
...
(
	trap "kill 0" SIGINT
	exec "./$gameserver_file" -rootPath= -gameRootPath="$gamedata_path" -legacyHTDOCSPath="$legacy_path" &
	curl -X POST -d "{\"filePath\":\"$entry_gamedata_file\"}" http://localhost:22501/fpProxy/api/mountzip &
